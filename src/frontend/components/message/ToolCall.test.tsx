...
import { ToolCall, getToolSummary, formatToolInput } from "./ToolCall.tsx";
...
describe("getToolSummary", () => {
  test("Read → file_path", () => {
    expect(
      getToolSummary(makeCall({ name: "Read", input: { file_path: "/src/app.ts" } }))
    ).toBe("/src/app.ts");
  });

  test("Write → file_path", () => {
    expect(
      getToolSummary(makeCall({ name: "Write", input: { file_path: "/out.ts" } }))
    ).toBe("/out.ts");
  });

  test("Edit → file_path", () => {
    expect(
      getToolSummary(makeCall({ name: "Edit", input: { file_path: "/edit.ts" } }))
    ).toBe("/edit.ts");
  });
  ...
  test("Glob → pattern", () => {
    expect(
      getToolSummary(makeCall({ name: "Glob", input: { pattern: "**/*.ts" } }))
    ).toBe("**/*.ts");
  });

  test("Grep → pattern truncated", () => {
    expect(
      getToolSummary(makeCall({ name: "Grep", input: { pattern: "searchTerm" } }))
    ).toBe("searchTerm");
  });

  test("Task → description truncated", () => {
    expect(
      getToolSummary(makeCall({ name: "Task", input: { description: "Find the bug" } }))
    ).toBe("Find the bug");
  });
  ...
  test("WebSearch → query truncated", () => {
    expect(
      getToolSummary(makeCall({ name: "WebSearch", input: { query: "bun test docs" } }))
    ).toBe("bun test docs");
  });
  ...
  test("Skill → skill name", () => {
    expect(
      getToolSummary(makeCall({ name: "Skill", input: { skill: "commit" } }))
    ).toBe("commit");
  });

  test("TaskCreate → subject", () => {
    expect(
      getToolSummary(makeCall({ name: "TaskCreate", input: { subject: "Fix login" } }))
    ).toBe("Fix login");
  });
  ...
  test("TaskUpdate → taskId + status", () => {
    expect(
      getToolSummary(
        makeCall({ name: "TaskUpdate", input: { taskId: "3", status: "completed" } })
      )
    ).toBe("#3 → completed");
  });

  test("TaskList → static text", () => {
    expect(getToolSummary(makeCall({ name: "TaskList", input: {} }))).toBe(
      "List all tasks"
    );
  });

  test("EnterPlanMode → static text", () => {
    expect(getToolSummary(makeCall({ name: "EnterPlanMode", input: {} }))).toBe(
      "Enter plan mode"
    );
  });

  test("ExitPlanMode → static text", () => {
    expect(getToolSummary(makeCall({ name: "ExitPlanMode", input: {} }))).toBe(
      "Exit plan mode"
    );
  });
  ...
  test("MCP tool → server > action", () => {
    expect(
      getToolSummary(makeCall({ name: "mcp__github__create_issue", input: {} }))
    ).toBe("create_issue");
  });
...
describe("formatToolInput", () => {
  ...
  test("Read → file path", () => {
    expect(
      formatToolInput(makeCall({ name: "Read", input: { file_path: "/b.ts" } }))
    ).toBe("/b.ts");
  });
  ...
  test("Default → JSON", () => {
    const result = formatToolInput(
      makeCall({ name: "SomeNewTool", input: { x: 1 } })
    );
