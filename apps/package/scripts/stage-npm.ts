...
if (srcPkg.dependencies) {
  for (const [name, ver] of Object.entries(srcPkg.dependencies as Record<string, string>)) {
    ...
    if (ver === "workspace:*" || ver.startsWith("workspace:")) continue;
    // Skip internal @cookielab.io packages
    if (name.startsWith("@cookielab.io/klovi-")) continue;
    deps[name] = ver;
  }
}
...
const workspaceImportPattern = /@cookielab\.io\/klovi-/;
const hasWorkspaceImports =
  workspaceImportPattern.test(cliContent) || workspaceImportPattern.test(serverContent);

if (hasWorkspaceImports) {
  // biome-ignore lint/suspicious/noConsole: CLI script output
  console.error(
    "Error: Built files still contain references to internal workspace packages (@cookielab.io/klovi-*).",
  );
  // biome-ignore lint/suspicious/noConsole: CLI script output
...
console.log(
  "  dist/server.js:",
  existsSync(resolve(stageDir, "dist/server.js")) ? "OK" : "MISSING",
);
// biome-ignore lint/suspicious/noConsole: CLI script output
