...
const DARK_SUFFIX_RE = /-dark/i;
const DARK_VARIANT_RE = /:dark/i;
...
export async function detectLinuxSystemTheme(
  ...
  try {
    const result = await Bun.$`gsettings get org.gnome.desktop.interface color-scheme`
      .text()
      .then((t) => t.trim());
  ...
  try {
    const result = await Bun.$`gsettings get org.gnome.desktop.interface gtk-theme`
      .text()
      .then((t) => t.trim());
