...
async function resolveZstdPath(appPath: string, explicitPath?: string): Promise<string> {
  ...
  throw new Error(
    `Required binary not found. Looked for: ${getDefaultZstdPaths(appPath).join(", ")}`,
  );
}
...
export async function verifyMacOSWrapperContract(args: VerifyArgs): Promise<void> {
  ...
  try {
    const decompressProc = Bun.spawn(
    ...
    if (!topLevelEntries.has(expectedBundleName)) {
      ...
      const actualSummary =
        actualBundleNames.length > 0 ? actualBundleNames.join(", ") : "(no .app entries found)";
