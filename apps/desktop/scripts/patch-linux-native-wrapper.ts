...
export async function patchNativeWrapperLibrary(libraryPath: string): Promise<PatchResult> {
  ...
  const { alreadyPatched, bytes, replacements } = patchNativeWrapperBytes(
...
export async function patchLinuxNativeWrapper(args: PatchArgs): Promise<PatchResult[]> {
  ...
  const results = await Promise.all(
    libraryPaths.map((libraryPath) => patchNativeWrapperLibrary(libraryPath)),
  );
  ...
  const alreadyPatchedCount = results.filter(
    (result) => result.status === "already_patched",
  ).length;
  ...
  console.log(
    `Patched Linux native wrapper WM_CLASS: ${patchedCount} updated, ${alreadyPatchedCount} already patched`,
  );
  return results;
}
