...
function parseArgs(argv: string[]): VerifyUpdaterArtifactArgs {
	...
	for (const arg of iter) {
		switch (arg) {
			case "--platform":
				platform = parsePlatform(iter.next().value);
				break;
			case "--arch":
				arch = parseArch(iter.next().value);
				break;
			case "--version":
				version = iter.next().value;
				break;
			case "--tarball":
				tarballPath = iter.next().value;
				break;
			case "--update-json":
				updateJsonPath = iter.next().value;
				break;
			case "--zstd":
				zstdPath = iter.next().value;
				break;
			default:
...
async function resolveZstdPath(platform: Platform, arch: Arch, explicitPath?: string): Promise<string> {
	if (explicitPath) {
	...
	throw new Error(`Required binary not found. Looked for: ${getDefaultZstdPaths(platform, arch).join(", ")}`);
}
...
async function decompressTarball(args: VerifyUpdaterArtifactArgs, tempDir: string): Promise<string> {
	...
	const proc = Bun.spawn([zstdPath, "decompress", "-i", tarballPath, "-o", tarPath, "--no-timing"], {
		stdout: "pipe",
		stderr: "pipe",
	});
