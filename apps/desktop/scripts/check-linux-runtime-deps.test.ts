...
async function writeBundle(root: string): Promise<{
	...
	return { launcherPath: launcherPath, localLibPath: localLibPath, wrapperPath: wrapperPath };
}
...
describe("checkLinuxRuntimeDeps", () => {
	...
	it("reports only unresolved system dependencies", async () => {
		...
		const runner: CommandRunner = async () => ({
			exitCode: 0,
			stderr: "",
			stdout: [`\tlibasar.so => ${localLibPath} (0x1234)`, "\tlibwebkit2gtk-4.1.so.0 => not found", ""].join("\n"),
		});

		await expect(checkLinuxRuntimeDeps({ bundlePath: root }, runner)).rejects.toThrow("libwebkit2gtk-4.1.so.0");
	});
});
