import { cleanCommandMessage, parseCommandMessage } from "./command-message";

describe("cleanCommandMessage", () => {
	it("with command tags → returns args text", () => {
		const input =
			"<command-message>commit</command-message><command-name>/commit</command-name><command-args>fix: resolve login bug</command-args>";
		expect(cleanCommandMessage(input)).toBe("fix: resolve login bug");
	});

	it("with no args → returns command name", () => {
		const input =
			"<command-message>superpowers:using-superpowers</command-message><command-name>/superpowers:using-superpowers</command-name>";
		expect(cleanCommandMessage(input)).toBe("/superpowers:using-superpowers");
	});

	it("with plain text → returns unchanged", () => {
		expect(cleanCommandMessage("Hello world")).toBe("Hello world");
	});

	it("with message tag only → strips tag wrapper", () => {
		const input = "<command-message>help</command-message>";
		expect(cleanCommandMessage(input)).toBe("");
	});
});

describe("parseCommandMessage", () => {
	it("with full tags → returns { name, args }", () => {
		const input =
			"<command-message>feature-dev</command-message><command-name>/feature-dev</command-name><command-args>implement login</command-args>";
		const result = parseCommandMessage(input);
		expect(result).toEqual({ name: "/feature-dev", args: "implement login" });
	});

	it("with missing args → returns { name, args: '' }", () => {
		const input = "<command-message>help</command-message><command-name>/help</command-name>";
		const result = parseCommandMessage(input);
		expect(result).toEqual({ name: "/help", args: "" });
	});

	it("with plain text → returns null", () => {
		expect(parseCommandMessage("Just a regular message")).toBeNull();
	});

	it("with empty command tags → returns null", () => {
		const input = "<command-message>help</command-message><command-name> </command-name><command-args> </command-args>";
		expect(parseCommandMessage(input)).toBeNull();
	});
});
