...
describe("buildTurns", () => {
	...
	it("bash-stdout + bash-stderr → both fields set", () => {
		const lines: RawLine[] = [
			line({
				...
				message: {
					role: "user",
					content: "<bash-stdout>To github.com:repo.git</bash-stdout><bash-stderr>warning: something</bash-stderr>",
				},
			}),
		];
		const turns = buildTurns(lines);
	...
	it("bash-input followed by bash-stdout+stderr merges with all fields", () => {
		const lines: RawLine[] = [
			...
			line({
				...
				message: {
					role: "user",
					content: "<bash-stdout>added 5 packages</bash-stdout><bash-stderr>warn deprecated</bash-stderr>",
				},
			}),
		];
		const turns = buildTurns(lines);
	...
	it("empty/malformed lines handled gracefully", () => {
		...
		const parseError = turns[1] as ParseErrorTurn;
