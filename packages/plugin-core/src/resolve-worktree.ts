...
const T3CODE_SUFFIX_REGEX = /\/t3code-[a-f0-9]+$/;
const GITDIR_WORKTREE_REGEX = /^gitdir:\s*(.+?)\s*$/;
...
export function stripT3CodeSuffix(path: string): { path: string; projectName: string } | null {
  if (!T3CODE_SUFFIX_REGEX.test(path)) return null;
...
export function resolveGitWorktree(worktreePath: string) {
  return Effect.gen(function* () {
    ...
    if (!info || info.type !== "File") return worktreePath;

    const content = yield* fs
      .readFileString(dotGitPath)
      .pipe(Effect.catchAll(() => Effect.succeed("")));
    if (!content) return worktreePath;

    const match = GITDIR_WORKTREE_REGEX.exec(content);
    if (!match?.[1]) return worktreePath;

    const gitdir = match[1];
    const worktreeIdx = gitdir.indexOf(GIT_WORKTREES_SEGMENT);
    if (worktreeIdx === -1) return worktreePath;
...
function buildNameMap<T extends { resolvedPath: string }>(
  ...
  for (const project of projects) {
    if (t3codeProjects.has(project)) continue;
    const name = lastSegment(project.resolvedPath);
    if (!name) continue;
    const existing = nameToResolvedPaths.get(name);
    if (existing) {
      if (!existing.includes(project.resolvedPath)) existing.push(project.resolvedPath);
    } else {
...
function resolveEntry<T extends { resolvedPath: string }>(
  ...
  return Effect.gen(function* () {
    ...
    if (candidates.length === 1) {
      ...
      if (resolvedPath) entry.project.resolvedPath = resolvedPath;
      return;
    }
...
export function resolveT3CodePaths<T extends { resolvedPath: string }>(projects: T[]) {
  return Effect.gen(function* () {
    ...
    if (t3codeEntries.length === 0) return;
