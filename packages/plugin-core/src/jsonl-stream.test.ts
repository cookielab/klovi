...
describe("streamJsonlHead", () => {
	it("invokes visitor for each parsed line in order", async () => {
		...
		await Bun.write(
			filePath,
			[JSON.stringify({ a: 1 }), JSON.stringify({ a: 2 }), JSON.stringify({ a: N_3 })].join("\n"),
		);
	...
	it("respects maxLines cap", async () => {
		...
		const lines = Array.from({ length: N_50 }, (_, i) => JSON.stringify({ i: i }));
	...
	it("does not load full file when bailing on line 2 of a large file", async () => {
		...
		const padding = JSON.stringify({ pad: "x".repeat(N_1000) });
		const lineCount = N_5000;
		...
		expect(after - before).toBeLessThan(N_1024 * N_1024);
	});
	...
	it("calls onMalformed for bad JSON and continues", async () => {
		...
		await Bun.write(filePath, [JSON.stringify({ a: 1 }), "{ not json", JSON.stringify({ a: N_3 })].join("\n"));
...
describe("streamJsonl", () => {
	...
	it("preserves visit order across chunk boundaries", async () => {
		...
		const lines = Array.from({ length: N_500 }, (_, i) => JSON.stringify({ i: i, pad: "x".repeat(N_50) }));
		await Bun.write(filePath, lines.join("\n"));
	...
	it("calls onMalformed for bad lines and continues to the end", async () => {
		...
		expect(seen).toEqual([1, N_3, N_5]);
		expect(errs).toEqual([2, N_4]);
	});
});
