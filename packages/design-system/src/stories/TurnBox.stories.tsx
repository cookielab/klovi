	...
	<div>
		{ROLES.map((role) => (
			<TurnBox
				...
				timestamp={
					<span>
						<Text>{T_2_MIN_AGO}</Text>
					</span>
				}
			>
				<p>
					<Text>{T_THIS_IS_A}</Text>
					<Text>{T_SP_1}</Text>
					{role}
					<Text>{T_SP_1}</Text>
					<Text>{T_MESSAGE_THE_LEFT_BORDER_AND_BA}</Text>
				</p>
			</TurnBox>
		))}
	</div>
...
export const WithModel: Story = () => (
	<div>
		<TurnBox
			...
			timestamp={
				<span>
					<Text>{T_JUST_NOW}</Text>
				</span>
			}
		>
			<p>
				<Text>{T_THIS_ASSISTANT_MESSAGE_INCLUDE}</Text>
			</p>
		</TurnBox>
	</div>
);
...
export default { title: "Components/TurnBox" } satisfies StoryDefault;
