import { isClaudeModel, shortModel } from "./model";

describe("shortModel", () => {
	it("returns family with major.minor for versioned model IDs", () => {
		expect(shortModel("claude-opus-4-6")).toBe("Opus 4.6");
		expect(shortModel("claude-sonnet-4-5-20250929")).toBe("Sonnet 4.5");
		expect(shortModel("claude-haiku-4-5-20251001")).toBe("Haiku 4.5");
	});

	it("returns family with major only for date-suffixed model IDs", () => {
		expect(shortModel("claude-opus-4-20250514")).toBe("Opus 4");
	});

	it("shortens GPT model names", () => {
...
describe("isClaudeModel", () => {
	...
	it("returns false for empty and unknown strings", () => {
